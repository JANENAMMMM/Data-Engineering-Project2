...
def step5_classifier(masked_results: list[dict], index_rows: list[dict]) -> list[dict]:
    ...
    try:
        ...
        clf_results = clf.predict_batch(images)
        for row, clf_res in zip(rows, clf_results):
            if clf_res:
                ...
                print(f"  {PASS} {row['file_id']}: pp={row['pattern_position']} ps={row['pattern_size']} trim={row['trim']}")
