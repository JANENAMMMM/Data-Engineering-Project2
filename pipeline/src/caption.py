    ...
    for attempt in range(retries):
        try:
            response = client.models.generate_content(
                model=GEMINI_MODEL,
                contents=_build_contents(img_bytes, mime_type, label_hint),
                config=_make_config(),
            )
...
async def _process_image(
    ...
    hint = (label_hints or {}).get(stem)

    async with sem:
        try:
            img_bytes = await asyncio.to_thread(img_path.read_bytes)
            vlm = await _call_gemini_async(img_bytes, "image/jpeg", label_hint=hint)
...
async def batch_from_dir_async(
    ...
    파일명 {file_id}_{category}.jpg 에서 메타데이터 추출.
    체크포인트: out_path가 이미 존재하면 완료된 (file_id, category) 쌍을 스킵.

    label_hints: {stem: "대분류: 상의\\n카테고리: 블라우스"}
        stem = "{file_id}_{category_en}" (예: "1028690_top")
        제공된 이미지는 Gemini 요청 시 사전 정보로 함께 전달됨.
    """
    ...
    if label_hints:
        hint_count = sum(1 for p in remaining if p.stem in label_hints)
        print(f"label_hints 적용 대상: {hint_count}/{len(remaining)}개")
    ...
    async def worker():
        while True:
            try:
                ...
                return
            await _process_image(img_path, sem, out_p, counters, write_lock, label_hints)
...
def batch_from_dir(
    ...
    asyncio.run(batch_from_dir_async(
        images_dir, out_path,
        limit=limit, concurrency=concurrency, label_hints=label_hints,
    ))
